...
const Btn     = ({children,onClick,primary,small,disabled,style}) => <button onClick={onClick} disabled={disabled} style={{background:primary?'#0f0f0f':'#fff',color:primary?'#fff':'#0f0f0f',border:primary?'none':'1.5px solid #e0e0da',borderRadius:10,padding:small?'5px 11px':'8px 16px',fontWeight:600,fontSize:small?11:13,fontFamily:"'Syne',sans-serif",cursor:disabled?'not-allowed':'pointer',opacity:disabled?.5:1,whiteSpace:'nowrap',...style}}>{children}</button>
...
const G2      = ({children,gap,mobile}) => <div style={{display:'grid',gridTemplateColumns:mobile?'1fr':'1fr 1fr',gap:gap||12}}>{children}</div>
...
function Modal({open,onClose,title,children,wide}){
  if(!open) return null
  return <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:999,padding:0}}>
    <div onClick={e=>e.stopPropagation()} style={{background:'#fafaf8',borderRadius:'20px 20px 0 0',padding:'20px 20px 32px',width:'100%',maxWidth:wide?700:560,maxHeight:'92vh',overflowY:'auto',border:'1.5px solid #e8e8e2'}}>
      <div style={{width:40,height:4,background:'#e0e0da',borderRadius:4,margin:'0 auto 20px'}}/>
...
function OpForm({accounts,clients,credits,onSubmit,onClose,editOp}){
  ...
  const submit = () => {
    ...
    else if(mode==='credit_out'){if(!fa||!ca){alert('Ingresá ambos montos');return};legs.push({id:uid(),kind:'account',accId:f.fromAccId,cur:fromAcc?.currency,delta:-fa});legs.push({id:uid(),kind:'credit',clientId:f.clientId,clientName:client?.name,cur:f.creditCur,delta:ca})}
    else if(mode==='credit_in'){if(!ca||!ta){alert('Ingresá ambos montos');return};legs.push({id:uid(),kind:'credit',clientId:f.clientId,clientName:client?.name,cur:f.creditCur,delta:-ca});legs.push({id:uid(),kind:'account',accId:f.toAccId,cur:toAcc?.currency,delta:ta})}
    ...
    onSubmit({id:editOp?editOp.id:uid(),createdAt:new Date(f.date).toISOString(),detail:f.detail,mode,clientId:f.clientId||null,clientName:client?.name||null,rate:parseFloat(f.rate)||null,profit:autoProfit,legs},editOp)
    onClose()
  }
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:12}}>
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
      {MODES.map(m=><button key={m.k} onClick={()=>setMode(m.k)} style={{background:mode===m.k?'#0f0f0f':'#f8f8f4',color:mode===m.k?'#fff':'#555',border:'1.5px solid '+(mode===m.k?'#0f0f0f':'#e8e8e2'),borderRadius:10,padding:'8px 10px',cursor:'pointer',textAlign:'left'}}>
    ...
    {mode==='exchange'&&<>
      <div style={{background:'#fff5f5',border:'1.5px solid #fecaca',borderRadius:12,padding:'10px 12px'}}>
        ...
        <G2 mobile={mobile}><Sel label="Cuenta" value={f.fromAccId} onChange={e=>set('fromAccId',e.target.value)}>{accOpts}</Sel><Inp label={'Monto ('+(fromAcc?.currency||'—')+')'} type="number" placeholder="0.00" value={f.fromAmt} onChange={e=>set('fromAmt',e.target.value)}/></G2>
      </div>
      <div style={{background:'#f0fdf4',border:'1.5px solid #86efac',borderRadius:12,padding:'10px 12px'}}>
        ...
        <G2 mobile={mobile}><Sel label="Cuenta" value={f.toAccId} onChange={e=>set('toAccId',e.target.value)}>{accOpts}</Sel><Inp label={'Monto ('+(toAcc?.currency||'—')+')'} type="number" placeholder="0.00" value={f.toAmt} onChange={e=>set('toAmt',e.target.value)}/></G2>
      </div>
    ...
    {mode==='credit_out'&&<>
      <div style={{background:'#fff5f5',border:'1.5px solid #fecaca',borderRadius:12,padding:'10px 12px'}}>
        ...
        <G2 mobile={mobile}><Sel label="Tu cuenta" value={f.fromAccId} onChange={e=>set('fromAccId',e.target.value)}>{accOpts}</Sel><Inp label={'Monto ('+(fromAcc?.currency||'—')+')'} type="number" placeholder="0.00" value={f.fromAmt} onChange={e=>set('fromAmt',e.target.value)}/></G2>
      </div>
      <div style={{background:'#fffbeb',border:'1.5px solid #fcd34d',borderRadius:12,padding:'10px 12px'}}>
        ...
        <div style={{marginTop:8}}><G2 mobile={mobile} gap={8}><Inp label="Monto que te debe" type="number" placeholder="0.00" value={f.creditAmt} onChange={e=>set('creditAmt',e.target.value)}/><Sel label="Moneda" value={f.creditCur} onChange={e=>set('creditCur',e.target.value)}>{CURRENCIES.map(c=><option key={c}>{c}</option>)}</Sel></G2></div>
    ...
    {mode==='credit_in'&&<>
      ...
      {Object.entries(existCreds).filter(([,v])=>v>0).length>0&&<div style={{background:'#fffbeb',border:'1.5px solid #fcd34d',borderRadius:10,padding:'10px 12px',fontSize:12}}><div style={{fontWeight:700,marginBottom:4,color:'#b45309'}}>Deuda pendiente:</div>{Object.entries(existCreds).filter(([,v])=>v>0).map(([c,v])=><div key={c} style={{display:'flex',justifyContent:'space-between'}}><span style={{color:'#888'}}>{c}</span><span style={{fontWeight:700}}>{fmt(v,c)}</span></div>)}</div>}
      <div style={{background:'#fffbeb',border:'1.5px solid #fcd34d',borderRadius:12,padding:'10px 12px'}}>
        ...
        <G2 mobile={mobile} gap={8}><Inp label="Monto" type="number" placeholder="0.00" value={f.creditAmt} onChange={e=>set('creditAmt',e.target.value)}/><Sel label="Moneda" value={f.creditCur} onChange={e=>set('creditCur',e.target.value)}>{CURRENCIES.map(c=><option key={c}>{c}</option>)}</Sel></G2>
      </div>
      <div style={{background:'#f0fdf4',border:'1.5px solid #86efac',borderRadius:12,padding:'10px 12px'}}>
        <div style={{fontSize:10,fontWeight:800,color:'#16a34a',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.05em'}}>📥 Entra a tu cuenta</div>
        <G2 mobile={mobile}><Sel label="Cuenta" value={f.toAccId} onChange={e=>set('toAccId',e.target.value)}>{accOpts}</Sel><Inp label={'Monto ('+(toAcc?.currency||'—')+')'} type="number" placeholder="0.00" value={f.toAmt} onChange={e=>set('toAmt',e.target.value)}/></G2>
    ...
    {mode==='transfer'&&<div style={{background:'#f5f3ff',border:'1.5px solid #c4b5fd',borderRadius:12,padding:'10px 12px'}}>
      ...
      <G2 mobile={mobile}><Sel label="Origen" value={f.fromAccId} onChange={e=>set('fromAccId',e.target.value)}>{accOpts}</Sel><Sel label="Destino" value={f.toAccId} onChange={e=>set('toAccId',e.target.value)}>{accOpts}</Sel></G2>
      <div style={{marginTop:8}}><G2 mobile={mobile}><Inp label={'Sale ('+(fromAcc?.currency||'—')+')'} type="number" placeholder="0.00" value={f.fromAmt} onChange={e=>set('fromAmt',e.target.value)}/><Inp label={'Entra ('+(toAcc?.currency||'—')+')'} type="number" placeholder="igual si misma moneda" value={f.toAmt} onChange={e=>set('toAmt',e.target.value)}/></G2></div>
    </div>}
    ...
    {autoProfit!==null&&<div style={{background:autoProfit>=0?'#f0fdf4':'#fef2f2',border:'1.5px solid '+(autoProfit>=0?'#86efac':'#fca5a5'),borderRadius:10,padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
      <span style={{fontSize:12,fontWeight:600,color:'#666'}}>Profit/Loss (USD)</span>
      <span style={{fontWeight:800,fontSize:18,fontFamily:"'Syne',sans-serif",color:autoProfit>=0?'#16a34a':'#dc2626'}}>{(autoProfit>=0?'+':'')+fmt(autoProfit,'USD')}</span>
    </div>}
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:14}}>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:10}}>
      <div style={{background:ACC_COLOR[acc.type]||'#f0f0ea',borderRadius:12,padding:'12px 14px'}}>
        ...
        <div style={{fontSize:20,fontWeight:800,fontFamily:"'Syne',sans-serif"}}>{fmt(accBal(acc),acc.currency)}</div>
      ...
      <div style={{background:'#f8f8f4',borderRadius:12,padding:'12px 14px'}}>
        ...
        {editOB
          ...
          :<div style={{display:'flex',alignItems:'center',gap:8}}><div style={{fontSize:16,fontWeight:800,fontFamily:"'Syne',sans-serif"}}>{fmt(acc.opening_bal||0,acc.currency)}</div><Btn small onClick={()=>setEditOB(true)}>✏</Btn></div>
        }
      </div>
    </div>
    <div style={{overflowX:'auto',borderRadius:12,border:'1.5px solid #e8e8e2'}}>
      <table style={{borderCollapse:'collapse',width:'100%',minWidth:400}}>
        ...
        <tbody>
          ...
          {accOps.length===0?<tr><td colSpan={5} style={{padding:'2rem',textAlign:'center',color:'#ccc'}}>Sin movimientos aún</td></tr>:accOps.map(row=>{running+=row.delta;const snap=running;return<tr key={row.id}><td style={{...TD_S,fontSize:10,color:'#aaa',whiteSpace:'nowrap'}}>{fmtDT(row.date)}</td><td style={TD_S}><Tag>{modeLab[row.mode]||'—'}</Tag></td><td style={{...TD_S,maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{row.detail||'—'}{row.clientName&&<span style={{display:'block',fontSize:10,background:cliColor(row.clientName),padding:'0 5px',borderRadius:8,marginTop:2,width:'fit-content'}}>{row.clientName}</span>}</td><td style={{...TD_S,fontWeight:700,color:row.delta>=0?'#16a34a':'#dc2626',whiteSpace:'nowrap'}}>{(row.delta>=0?'+':'')+fmt(row.delta,acc.currency)}</td><td style={{...TD_S,fontWeight:800,fontFamily:"'Syne',sans-serif",color:snap<0?'#dc2626':'#0f0f0f',whiteSpace:'nowrap'}}>{fmt(snap,acc.currency)}</td></tr>})}
        </tbody>
      </table>
    </div>
  </div>
...
function Sidebar({tab,setTab,profile,signOut,mobile,menuOpen,setMenuOpen}){
  if(mobile){
    return <>
      {/* Bottom nav */}
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:'#0f0f0f',display:'flex',zIndex:200,paddingBottom:'env(safe-area-inset-bottom)'}}>
        {TABS.map(t=><button key={t} onClick={()=>{setTab(t);setMenuOpen(false)}} style={{flex:1,background:tab===t?T_COL[t]:'transparent',color:tab===t?'#0f0f0f':'#555',border:'none',padding:'10px 0 8px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:2}}>
          <span style={{fontSize:16}}>{T_ICON[t]}</span>
          <span style={{fontSize:9,fontWeight:tab===t?700:400,fontFamily:"'DM Sans',sans-serif"}}>{t}</span>
        </button>)}
      </div>
      {/* Top bar */}
      <div style={{position:'sticky',top:0,background:'#0f0f0f',zIndex:100,padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:15,color:'#fff',letterSpacing:'-0.02em'}}>SATO TREASURY</div>
  ...
  return <div style={{width:175,minWidth:175,background:'#0f0f0f',display:'flex',flexDirection:'column',padding:'20px 0',position:'sticky',top:0,height:'100vh',flexShrink:0}}>
    ...
    <nav style={{display:'flex',flexDirection:'column',gap:2,padding:'14px 8px',flex:1}}>
      {TABS.map(t=><button key={t} onClick={()=>setTab(t)} style={{background:tab===t?T_COL[t]:'transparent',color:tab===t?'#0f0f0f':'#555',border:'none',borderRadius:10,padding:'9px 12px',cursor:'pointer',textAlign:'left',fontWeight:tab===t?700:400,fontSize:12,fontFamily:"'DM Sans',sans-serif",display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:13}}>{T_ICON[t]}</span>{t}</button>)}
    </nav>
    <div style={{padding:'12px 16px',borderTop:'1px solid #222'}}>
      <div style={{fontSize:12,color:'#555',marginBottom:4,fontWeight:500}}>{profile?.name}</div>
...
function Dashboard({accounts,clients,ops,credits,setTab}){
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:16}}>
    ...
    <div style={{background:'#0f0f0f',borderRadius:16,padding:mobile?'16px':'20px 24px',color:'#fff'}}>
      ...
      <div style={{fontSize:mobile?28:34,fontWeight:800,fontFamily:"'Syne',sans-serif",letterSpacing:'-0.03em',color:nw>=0?'#86efac':'#fca5a5'}}>{(nw>=0?'+':'')+fmt(nw,'USD')}</div>
    ...
    <div style={{display:'grid',gridTemplateColumns:cols,gap:14}}>
      ...
      <Card>
        ...
        {credSumm.length===0?<div style={{textAlign:'center',padding:'12px 0'}}><div style={{fontSize:22}}>✓</div><div style={{fontSize:12,color:'#888',marginTop:4}}>Sin créditos</div></div>:
        credSumm.map(({c,debts})=><div key={c.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 10px',marginBottom:6,background:'#fffbeb',borderRadius:10,border:'1px solid #fcd34d'}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}><div style={{width:26,height:26,borderRadius:'50%',background:cliColor(c.name),display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:800,fontFamily:"'Syne',sans-serif",flexShrink:0}}>{c.name.slice(0,2)}</div><span style={{fontWeight:600,fontSize:13,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:80}}>{c.name}</span></div>
          <div style={{textAlign:'right',flexShrink:0}}>{debts.map(([cur,v])=><div key={cur} style={{fontSize:12,fontWeight:700,color:'#b45309'}}>{fmt(v,cur)}</div>)}</div>
        </div>)}
      </Card>
    </div>
    ...
    <Card>
      ...
      {recent.length===0?<div style={{fontSize:12,color:'#ccc',textAlign:'center',padding:'14px 0'}}>Sin operaciones aún</div>:
      recent.map(op=><div key={op.id} style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',padding:'8px 0',borderBottom:'1px solid #f0f0ea'}}>
        <div style={{minWidth:0,flex:1,marginRight:8}}>
          <div style={{display:'flex',alignItems:'center',gap:4,flexWrap:'wrap',marginBottom:2}}><Tag bg="#f0f0ea">{modeLab[op.mode]||op.mode}</Tag>{op.clientName&&<Tag bg={cliColor(op.clientName)}>{op.clientName}</Tag>}</div>
...
// ── Operaciones ───────────────────────────────────────────────────────────────
function Operaciones({accounts,clients,credits,ops,onAddOp,onEditOp,isOperator}){
  const mobile=useIsMobile()
  const[open,setOpen]=useState(false),[editOp,setEdit]=useState(null),[q,setQ]=useState(''),[page,setPage]=useState(1)
  const PER=15
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:12}}>
    ...
    <Modal open={open} onClose={()=>{setOpen(false);setEdit(null)}} title={editOp?'Editar':'Nueva operación'}>
    ...
    {mobile
      ...
      ?<div style={{display:'flex',flexDirection:'column',gap:8}}>
        {paged.length===0?<div style={{textAlign:'center',color:'#ccc',padding:'2rem'}}>Sin operaciones</div>:
        paged.map(op=>{const out=op.legs?.filter(l=>l.delta<0)||[],inA=op.legs?.filter(l=>l.kind==='account'&&l.delta>0)||[],inC=op.legs?.filter(l=>l.kind==='credit'&&l.delta>0)||[]
          return <Card key={op.id} style={{padding:'12px 14px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
              <div><Tag bg={modeBg[op.mode]||'#f0f0ea'}>{modeLab[op.mode]||'—'}</Tag>{op.clientName&&<span style={{marginLeft:4}}><Tag bg={cliColor(op.clientName)}>{op.clientName}</Tag></span>}</div>
              <Btn small onClick={()=>{setEdit(op);setOpen(true)}}>✏</Btn>
            </div>
            {op.detail&&<div style={{fontSize:12,color:'#888',marginBottom:6,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{op.detail}</div>}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div>{out.map(l=><div key={l.id} style={{fontSize:12,color:'#dc2626',fontWeight:700}}>-{fmt(Math.abs(l.delta),l.cur)}</div>)}{inA.map(l=><div key={l.id} style={{fontSize:12,color:'#16a34a',fontWeight:700}}>+{fmt(l.delta,l.cur)}</div>)}{inC.map(l=><div key={l.id} style={{fontSize:12,color:'#b45309',fontWeight:700}}>💳 {fmt(l.delta,l.cur)}</div>)}</div>
              <div style={{textAlign:'right'}}>{op.profit!=null&&<Tag bg={op.profit>=0?'#dcfce7':'#fee2e2'} color={op.profit>=0?'#166534':'#991b1b'}>{(op.profit>=0?'+':'')+fmt(op.profit,'USD')}</Tag>}<div style={{fontSize:10,color:'#aaa',marginTop:2}}>{fmtDT(op.createdAt)}</div></div>
            </div>
          </Card>
        })}
      </div>
      ...
      :<Card style={{padding:0,overflow:'hidden'}}>
        <div style={{overflowX:'auto'}}>
          <table style={{borderCollapse:'collapse',width:'100%',minWidth:520}}>
            ...
            <tbody>{paged.length===0?<tr><td colSpan={6} style={{padding:'3rem',textAlign:'center',color:'#ccc'}}>Sin operaciones</td></tr>:paged.map(op=>{const out=op.legs?.filter(l=>l.delta<0)||[],inA=op.legs?.filter(l=>l.kind==='account'&&l.delta>0)||[],inC=op.legs?.filter(l=>l.kind==='credit'&&l.delta>0)||[];return<tr key={op.id}><td style={{...TD_S,fontSize:10,color:'#aaa',whiteSpace:'nowrap'}}>{fmtDT(op.createdAt)}</td><td style={TD_S}><Tag bg={modeBg[op.mode]||'#f0f0ea'}>{modeLab[op.mode]||'—'}</Tag>{op.clientName&&<div style={{marginTop:3}}><Tag bg={cliColor(op.clientName)}>{op.clientName}</Tag></div>}{op.detail&&<div style={{fontSize:10,color:'#aaa',marginTop:2,maxWidth:130,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{op.detail}</div>}</td><td style={{...TD_S,whiteSpace:'nowrap'}}>{out.map(l=><div key={l.id}><div style={{color:'#dc2626',fontWeight:700}}>-{fmt(Math.abs(l.delta),l.cur)}</div>{l.kind==='account'&&<div style={{fontSize:10,color:'#aaa'}}>{accMap[l.accId]?.name||'—'}</div>}</div>)}</td><td style={{...TD_S,whiteSpace:'nowrap'}}>{inA.map(l=><div key={l.id}><div style={{color:'#16a34a',fontWeight:700}}>+{fmt(l.delta,l.cur)}</div><div style={{fontSize:10,color:'#aaa'}}>{accMap[l.accId]?.name||'—'}</div></div>)}{inC.map(l=><div key={l.id}><div style={{color:'#b45309',fontWeight:700}}>💳 {fmt(l.delta,l.cur)}</div><div style={{fontSize:10,color:'#aaa'}}>Crédito: {l.clientName}</div></div>)}</td><td style={TD_S}>{op.profit!=null?<Tag bg={op.profit>=0?'#dcfce7':'#fee2e2'} color={op.profit>=0?'#166534':'#991b1b'}>{(op.profit>=0?'+':'')+fmt(op.profit,'USD')}</Tag>:'—'}</td><td style={TD_S}><Btn small onClick={()=>{setEdit(op);setOpen(true)}}>✏</Btn></td></tr>})}</tbody>
          </table>
        </div>
      </Card>
    }
...
function ClienteDetalle({client,ops,accounts,credits}){
  const mobile=useIsMobile()
  const d=new Date(),[df,setDf]=useState(new Date(d.getFullYear(),d.getMonth(),1).toISOString().slice(0,10)),[dt,setDt]=useState(d.toISOString().slice(0,10))
  const accMap=useMemo(()=>{const m={};accounts.forEach(a=>{m[a.id]=a});return m},[accounts])
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:14}}>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(110px,1fr))',gap:10}}>
      ...
      {openCr.length>0&&<div style={{background:'#fffbeb',borderRadius:12,padding:'10px 12px',border:'1.5px solid #fcd34d'}}><div style={{fontSize:10,fontWeight:800,color:'#b45309',textTransform:'uppercase',marginBottom:3}}>Me debe</div>{openCr.map(([cur,v])=><div key={cur} style={{fontSize:15,fontWeight:800,fontFamily:"'Syne',sans-serif",color:'#b45309'}}>{fmt(v,cur)}</div>)}</div>}
    </div>
    <G2 mobile={mobile}><Inp label="Desde" type="date" value={df} onChange={e=>setDf(e.target.value)}/><Inp label="Hasta" type="date" value={dt} onChange={e=>setDt(e.target.value)}/></G2>
    <div style={{overflowX:'auto',borderRadius:12,border:'1.5px solid #e8e8e2'}}>
      <table style={{borderCollapse:'collapse',width:'100%',minWidth:380}}>
        <thead><tr>{['Fecha','Tipo','Detalle','Sale','Entra'].map(x=><th key={x} style={TH_S}>{x}</th>)}</tr></thead>
        <tbody>{inRange.length===0?<tr><td colSpan={5} style={{padding:'2rem',textAlign:'center',color:'#ccc'}}>Sin operaciones en este período</td></tr>:inRange.map(op=>{const out=op.legs?.filter(l=>l.delta<0)||[],inL=op.legs?.filter(l=>l.delta>0)||[];return<tr key={op.id}><td style={{...TD_S,color:'#aaa',whiteSpace:'nowrap',fontSize:10}}>{fmtD(op.createdAt)}</td><td style={TD_S}><Tag>{modeLab[op.mode]||'—'}</Tag></td><td style={{...TD_S,maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontWeight:500}}>{op.detail||'—'}</td><td style={TD_S}>{out.map(l=><div key={l.id} style={{color:l.kind==='account'?'#dc2626':'#b45309',fontWeight:700,whiteSpace:'nowrap'}}>{(l.kind==='credit'?'💳 ':'-')+fmt(Math.abs(l.delta),l.cur)}</div>)}</td><td style={TD_S}>{inL.map(l=><div key={l.id} style={{color:l.kind==='account'?'#16a34a':'#b45309',fontWeight:700,whiteSpace:'nowrap'}}>{(l.kind==='credit'?'💳 ':'+')+fmt(l.delta,l.cur)}</div>)}</td></tr>})}</tbody>
      </table>
    </div>
  </div>
}

function Clientes({accounts,clients,ops,credits,onAddClient}){
  const mobile=useIsMobile()
  const[open,setOpen]=useState(false),[sel,setSel]=useState(null),[f,sf]=useState({name:'',phone:'',notes:''}),[q,setQ]=useState('')
  const filtered=clients.filter(c=>c.name.toLowerCase().includes(q.toLowerCase()))
  const submit=()=>{if(!f.name.trim())return;onAddClient(f);sf({name:'',phone:'',notes:''});setOpen(false)}
  return <div style={{display:'flex',flexDirection:'column',gap:12}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
      <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:mobile?18:22}}>Clientes</h2>
      <Btn primary onClick={()=>setOpen(true)}>+ Nuevo</Btn>
    </div>
    <Modal open={open} onClose={()=>setOpen(false)} title="Nuevo cliente">
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}><Btn onClick={()=>setOpen(false)}>Cancelar</Btn><Btn primary onClick={submit}>Guardar</Btn></div>
      </div>
    </Modal>
    ...
    <div style={{display:'grid',gridTemplateColumns:mobile?'1fr':'repeat(auto-fill,minmax(200px,1fr))',gap:10}}>
      ...
      {filtered.map(c=>{const cOps=clientOps(c.id,ops),profit=cOps.reduce((s,o)=>s+(o.profit||0),0),creds=cliCreds(c.id,credits),openCr=Object.entries(creds).filter(([,v])=>v>0),color=cliColor(c.name);return<Card key={c.id} onClick={()=>setSel(c)} style={{borderTop:'4px solid '+color}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}><div style={{width:34,height:34,borderRadius:'50%',background:color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,fontFamily:"'Syne',sans-serif",flexShrink:0}}>{c.name.slice(0,2)}</div><div style={{minWidth:0}}><div style={{fontWeight:700,fontSize:14,fontFamily:"'Syne',sans-serif",overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.name}</div><div style={{fontSize:11,color:'#888'}}>{cOps.length} operaciones</div></div></div>
        {openCr.length>0&&<div style={{background:'#fffbeb',border:'1px solid #fcd34d',borderRadius:8,padding:'5px 10px',marginBottom:8}}><div style={{fontSize:9,fontWeight:700,color:'#b45309',marginBottom:1}}>DEBE</div>{openCr.map(([cur,v])=><div key={cur} style={{fontWeight:800,fontFamily:"'Syne',sans-serif",fontSize:13,color:'#b45309'}}>{fmt(v,cur)+' '+cur}</div>)}</div>}
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'5px 8px',background:'#f8f8f4',borderRadius:8}}><span style={{fontSize:10,fontWeight:700,color:'#888',textTransform:'uppercase'}}>Profit</span><span style={{fontWeight:800,fontFamily:"'Syne',sans-serif",fontSize:14,color:profit>=0?'#16a34a':'#dc2626'}}>{(profit>=0?'+':'')+fmt(profit,'USD')}</span></div>
      </Card>})}
    </div>
  </div>
}
...
// ── Cuentas ───────────────────────────────────────────────────────────────────
function Cuentas({accounts,ops,onAddAccount,onUpdateOpeningBal,isOperator}){
  const mobile=useIsMobile()
  const[open,setOpen]=useState(false),[selAcc,setSel]=useState(null),[f,sf]=useState({name:'',type:'Efectivo',currency:'USD',titular:'',openingBal:0})
  const submit=()=>{if(!f.name.trim())return;onAddAccount({...f,openingBal:parseFloat(f.openingBal)||0});sf({name:'',type:'Efectivo',currency:'USD',titular:'',openingBal:0});setOpen(false)}
  return <div style={{display:'flex',flexDirection:'column',gap:12}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
      <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:mobile?18:22}}>Cuentas internas</h2>
      {isOperator&&<Btn primary onClick={()=>setOpen(true)}>+ Nueva</Btn>}
    </div>
    <Modal open={open} onClose={()=>setOpen(false)} title="Nueva cuenta">
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <G2 mobile={mobile}><Sel label="Tipo" value={f.type} onChange={e=>sf({...f,type:e.target.value})}>{ACC_TYPES.map(t=><option key={t}>{t}</option>)}</Sel><Sel label="Moneda" value={f.currency} onChange={e=>sf({...f,currency:e.target.value})}>{CURRENCIES.map(c=><option key={c}>{c}</option>)}</Sel></G2>
    ...
    {ACC_TYPES.map(type=>{const accs=accounts.filter(a=>a.type===type);if(!accs.length)return null;return<div key={type}>
      <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:8,marginTop:4}}><div style={{width:8,height:8,borderRadius:'50%',background:ACC_COLOR[type]||'#ccc'}}/><span style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:12,textTransform:'uppercase',letterSpacing:'0.05em'}}>{type}</span></div>
      <div style={{display:'grid',gridTemplateColumns:mobile?'1fr 1fr':'repeat(auto-fill,minmax(160px,1fr))',gap:8}}>
        {accs.map(acc=>{const bal=accBal(acc);return<Card key={acc.id} onClick={()=>setSel(acc)} style={{borderLeft:'4px solid '+(ACC_COLOR[acc.type]||'#ccc'),cursor:'pointer',padding:'12px 14px'}}>
          <div style={{fontWeight:700,fontSize:12,fontFamily:"'Syne',sans-serif",marginBottom:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{acc.name}</div>
          <div style={{fontSize:10,color:'#aaa',marginBottom:8}}>{acc.titular?acc.titular+' · ':''}{acc.currency}</div>
          <div style={{fontSize:20,fontWeight:800,fontFamily:"'Syne',sans-serif",color:bal<0?'#dc2626':'#0f0f0f'}}>{fmt(bal,acc.currency)}</div>
          <div style={{fontSize:9,color:'#bbb',marginTop:3}}>Ver historial →</div>
        </Card>})}
      </div>
    </div>})}
  </div>
...
function Reportes({accounts,clients,ops,credits}){
  const mobile=useIsMobile()
  const d=new Date(),visOps=ops.filter(o=>!o.isReversal)
  const[df,setDf]=useState(new Date(d.getFullYear(),d.getMonth(),1).toISOString().slice(0,10)),[dt,setDt]=useState(d.toISOString().slice(0,10)),[preset,setPreset]=useState('month')
  const applyP=p=>{setPreset(p);const n=new Date();if(p==='today'){setDf(n.toISOString().slice(0,10));setDt(n.toISOString().slice(0,10))}else if(p==='week'){const s=new Date(n);s.setDate(n.getDate()-6);setDf(s.toISOString().slice(0,10));setDt(n.toISOString().slice(0,10))}else if(p==='month'){setDf(new Date(n.getFullYear(),n.getMonth(),1).toISOString().slice(0,10));setDt(n.toISOString().slice(0,10))}else if(p==='year'){setDf(new Date(n.getFullYear(),0,1).toISOString().slice(0,10));setDt(n.toISOString().slice(0,10))}}
  ...
  const pByCli=useMemo(()=>{const m={};inRange.filter(o=>o.clientId&&o.profit).forEach(o=>{if(!m[o.clientId])m[o.clientId]={name:o.clientName,profit:0,ops:0};m[o.clientId].profit+=o.profit;m[o.clientId].ops++});return Object.values(m).sort((a,b)=>b.profit-a.profit)},[inRange])
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:14}}>
    ...
    <Card>
      ...
      <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:10}}>{PRESETS.map(p=><Btn key={p.k} small onClick={()=>applyP(p.k)} style={{background:preset===p.k?'#0f0f0f':'#f0f0ea',color:preset===p.k?'#fff':'#555',border:'none'}}>{p.l}</Btn>)}</div>
      <G2 mobile={mobile}><Inp label="Desde" type="date" value={df} onChange={e=>{setDf(e.target.value);setPreset('custom')}}/><Inp label="Hasta" type="date" value={dt} onChange={e=>{setDt(e.target.value);setPreset('custom')}}/></G2>
      <div style={{marginTop:8,fontSize:12,color:'#888'}}>{inRange.length} operaciones en el período</div>
    </Card>
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
      <Card style={{background:profit>=0?'#dcfce7':'#fee2e2',border:'none',padding:'12px 14px'}}><div style={{fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:4,fontFamily:"'Syne',sans-serif"}}>Profit período</div><div style={{fontSize:mobile?18:22,fontWeight:800,fontFamily:"'Syne',sans-serif",color:profit>=0?'#16a34a':'#dc2626'}}>{(profit>=0?'+':'')+fmt(profit,'USD')}</div></Card>
      <Card style={{background:'#f0f9ff',border:'none',padding:'12px 14px'}}><div style={{fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:4,fontFamily:"'Syne',sans-serif"}}>Operaciones</div><div style={{fontSize:mobile?18:22,fontWeight:800,fontFamily:"'Syne',sans-serif"}}>{inRange.length}</div></Card>
    </div>
    <div style={{display:'grid',gridTemplateColumns:cols,gap:14}}>
      <Card><div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:13,marginBottom:12}}>Volumen operado</div>{Object.keys(vol).length===0?<div style={{fontSize:12,color:'#ccc'}}>Sin operaciones</div>:Object.entries(vol).map(([cur,v])=><div key={cur} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #f0f0ea'}}><Tag>{cur}</Tag><span style={{fontWeight:800,fontFamily:"'Syne',sans-serif",fontSize:14}}>{fmt(v,cur)}</span></div>)}</Card>
...
export default function TreasuryApp(){
  ...
  const handleAddOp=async op=>{try{await addOperation(op);refetchOps();refetchAccounts();refetchCredits()}catch(e){alert('Error: '+e.message)}}
  const handleEditOp=async(op,orig)=>{try{await editOperation(op,orig);refetchOps();refetchAccounts();refetchCredits()}catch(e){alert('Error: '+e.message)}}
  const handleAddClient=async c=>{try{await addClient(c);refetchClients()}catch(e){alert('Error: '+e.message)}}
  const handleAddAccount=async a=>{try{await addAccount(a);refetchAccounts()}catch(e){alert('Error: '+e.message)}}
  const handleUpdateBal=async(id,bal)=>{try{await updateAccountOpeningBal(id,bal);refetchAccounts()}catch(e){alert('Error: '+e.message)}}

  return <div style={{display:'flex',minHeight:'100vh',fontFamily:"'DM Sans',sans-serif",flexDirection:mobile?'column':'row'}}>
    <style>{`*{box-sizing:border-box;margin:0;padding:0}body{background:#f4f4f0}`}</style>
    <Sidebar tab={tab} setTab={setTab} profile={profile} signOut={signOut} mobile={mobile} menuOpen={menuOpen} setMenuOpen={setMenuOpen}/>
    <main style={{flex:1,padding:mobile?'12px 12px 80px':'24px 28px',overflowY:'auto',overflowX:'hidden',maxWidth:'100%'}}>
      ...
      {tab==='Operaciones'&&<Operaciones accounts={accounts} clients={clients} credits={credits} ops={ops} onAddOp={handleAddOp} onEditOp={handleEditOp} isOperator={isOperator}/>}
      {tab==='Clientes'   &&<Clientes    accounts={accounts} clients={clients} ops={ops} credits={credits} onAddClient={handleAddClient} isOperator={isOperator}/>}
      {tab==='Cuentas'    &&<Cuentas     accounts={accounts} ops={ops} onAddAccount={handleAddAccount} onUpdateOpeningBal={handleUpdateBal} isOperator={isOperator}/>}
