...
export function useAccounts() {
  return useTable('account_balances')   // uses the view for pre-calculated balance
}
...
export function useOperations() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    // Fetch ops + their legs in one go
...
// Map DB snake_case → camelCase used in UI
...
export async function editOperation(op, originalOp) {
  // 1. Insert reversal of the original
  const reversal = {
    ...originalOp,
    id: undefined,
